...
Учебное задание по курсу **«Параллельное программирование»**.

Проект реализует построение гистограммы «доза–объём» (DVH) по 3D-сетке вокселей и демонстрирует использование параллельных вычислений через модуль `multiprocessing`.
...
Порядок следования доз по вокселям фиксирован, но **для задачи важен только набор значений**, а не их координаты (индексы (i, j, k) не используются). Все воксели считаются одинакового объёма.
